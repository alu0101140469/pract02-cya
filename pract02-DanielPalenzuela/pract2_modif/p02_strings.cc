...
/*
MODIF: añadir opcode 7 para calcular subcadena a partir de una cadena.
Cuando el resultado de la operacion es un conjunto de cadenas se genera como un lenguaje, asi que el resultado es el lenguaje que contiene las cadenas
de prefijos y sufijos.
Ej) abab -> {&, a, b, ab, ba, aba, bab, abab}
*/
...
bool EsOpcodeValido(const int opcode) {
  return opcode >= 1 && opcode <= 7; // modif
}
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    // modif
    std::cerr << "Error: el opcode debe ser un entero entre 1 y 7.\n";
    return 1;
  }

  if (!EsOpcodeValido(opcode)) {
    // modif
    std::cerr << "Error: el opcode debe estar entre 1 y 7.\n";
    return 1;
  }
