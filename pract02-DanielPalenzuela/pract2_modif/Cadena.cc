...
// modif
Lenguaje Cadena::Subcadenas() const {
  Lenguaje resultado;
  // añadir cadena vacia
  resultado.Insertar(Cadena("", alfabeto_));
  // Generar las subcadenas
  for (std::size_t start = 0; start < symbols_.size(); ++start) { // prueba cada posición como comienzo de la subcadena
    for (std::size_t length = 1;
         start + length <= symbols_.size();
         ++length) { // prueba diferentes tamaños desde esa posición
      resultado.Insertar(
          Cadena(symbols_.substr(start, length), alfabeto_)); // extrae subcadena
    }
  }
  return resultado;
}
